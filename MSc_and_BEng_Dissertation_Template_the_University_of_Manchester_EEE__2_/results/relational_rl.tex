...
\caption{UAV trajectories for one representative episode at $400{\times}400$, $N{=}30$ (seed 9). Both agents face the identical sensor layout. Green circles are sensors from which data was collected; red circles were not reached. The DQN collects only sensors within LoRa range of the start corner (NDR $= 33\%$). The relational policy navigates through the grid interior, reaching 28 of 30 sensors (NDR $= 93\%$). Aggregate statistics across 50 seeds are in Table~\ref{tab:scalability_ndr}.}
