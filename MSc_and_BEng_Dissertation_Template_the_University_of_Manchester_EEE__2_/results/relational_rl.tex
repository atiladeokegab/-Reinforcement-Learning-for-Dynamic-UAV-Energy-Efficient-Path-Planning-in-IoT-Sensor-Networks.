...
\caption{UAV trajectories for one representative episode at $300{\times}300$, $N{=}20$ (seed 1). Both agents face the identical sensor layout. Green circles are sensors from which data was collected; red circles were not reached. The DQN collects only the sensors within LoRa range of its start position, leaving 9 of 20 sensors unvisited. The relational policy navigates through the grid and visits all 20 sensors. Aggregate statistics across 50 seeds are in Table~\ref{tab:scalability_ndr}.}
