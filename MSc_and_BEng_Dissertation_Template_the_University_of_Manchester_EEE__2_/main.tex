...
\begin{document}
	...
	\title{Energy-Efficient UAV Path Planning in IoT Networks: A
		Deep Reinforcement Learning Aided Approach}
	\studentid{11085980}
	\renewcommand{\authortext}{\textbf{Student ID}\ \linebreak \@studentid}
	\makeatother
	...
	\supervisorname{Dr.\ Zahra Mobini}
	...
	\setcounter{table}{0} % longtables in abbreviations/symbols increment the counter; reset so main-body tables start at 1
	...
	\begin{uomappendix}
		...
		\input{appendices/project_outline}    % A.2 Project Outline
		\input{appendices/project_plan}       % A.2 Initial Project Plan
		\input{appendices/risk_register}      % A.3 Risk Register
		\input{appendices/cpd_log}            % A.4 CPD
		\input{appendices/hs_assessment}      % A.5 Health & Safety Risk Assessment
		\input{appendices/gantt_chart}        % A.6 Gantt Chart
		\input{appendices/repository}         % A.7 Source Code
		\input{appendices/hyperparams}        % A.8 Hyperparameters
		\input{appendices/sim_to_real}        % A.9 Sim-to-Real Analysis
		\input{appendices/additional_figures} % A.10 Additional Figures
