"""
train_transformer_relaxed.py
============================
A/B variant of train_transformer.py with a relaxed competence gate
(NDR >= 90 %) and a separate checkpoint / Ray-temp directory so it can
be run alongside (or in place of) the original without clobbering its
state. Everything else -- hyperparameters, model config, curriculum
stages, metric pipeline -- is identical to the canonical script.

Why relax the gate: on Stage 0 (100x100, N=10), the canonical 95% NDR
gate appears unreachable under the 10% per-step Tx probability plus
N(0, 4 dB) shadowing. The transformer plateaus around 87 % and stops
climbing, so the curriculum never advances and downstream stages
(Stages 1-4) never get tested. Dropping the gate to 90 % lets us
surface multi-stage results while keeping the original (stricter)
run available for comparison.

Outputs
-------
* Checkpoints:  models/transformer_gtrxl_relaxed/
* Ray tmp dir:  /workspace/ray_tmp_relaxed

Usage
-----
    PYTHONIOENCODING=utf-8 uv run python \\
        src/experiments/transformer_policy/train_transformer_relaxed.py
...
# RELAXED VARIANT: NDR gate lowered from 95 % to 90 % so Stage 0 can graduate
# even when the Bernoulli 10 % Tx probability caps achievable NDR.
ADVANCE_CRITERIA: dict[str, float] = {
    "ndr_pct": 90.0,   # New Discovery Rate threshold (%) — relaxed from 95
...
CHECKPOINT_ROOT = _ROOT / "models" / "transformer_gtrxl_relaxed"
...
def train() -> None:
    ...
    ray.init(
        ...
        num_cpus=8,
        num_gpus=NUM_GPUS,
        _temp_dir="/workspace/ray_tmp_relaxed",
        runtime_env={
