...
def save_sensor_snapshot(env, filepath, agent_name, is_wrapper=False):
    ...
    This data is used by fairness_analysis.py and buffer_heatmap_analysis.py.

    Args:
        env: UAVEnvironment instance (or AnalysisUAVEnv wrapper)
        filepath: Path for output JSON file
        agent_name: Agent identifier for metadata
        is_wrapper: True if env is AnalysisUAVEnv with last_episode_data
    """
    print(f"\n{'=' * 80}")
    print(f"Saving sensor snapshot: {agent_name}")
    print(f"{'=' * 80}")

    # Handle AnalysisUAVEnv wrapper with preserved data
    if (
        is_wrapper
    ...
    # Handle standard UAVEnvironment
    else:
    ...
    # Build complete snapshot with type conversion
    snapshot = {
        ...
        "mission_stats": {
            ...
            "coverage_percentage": float(
                (
                    len(
                        [s for s in sensor_data_list if s["total_data_transmitted"] > 0]
                    )
    ...
    # Process each sensor with derived metrics and type conversion
    ...
    print(
        f"  • Data collected: {snapshot['mission_stats']['total_data_collected']:.0f} bytes"
    )
...
DQN_CONFIG_PATH = (
    script_dir_results
    / "models"
    / "dqn_full_observability"
    / "frame_stacking_config.json"
)
...
def run_greedy_agent_for_plot(agent, env, name="Agent", seed=PLOT_CONFIG["seed"]):
    ...
    while not done:
        ...
        if env.current_step % 50 == 0 or done or truncated:
            ...
            efficiency = (
                (env.total_data_collected / energy_consumed)
...
def run_dqn_agent_for_plot(
    ...
    while not done:
        ...
        if unwrapped_env.current_step % 50 == 0 or done:
            ...
            coverage_pct = (
                (len(unwrapped_env.sensors_visited) / unwrapped_env.num_sensors) * 100
                if hasattr(unwrapped_env, "sensors_visited")
                else 0
            )
            energy_consumed = EVAL_MAX_BATTERY - unwrapped_env.uav.battery
            efficiency = (
                (unwrapped_env.total_data_collected / energy_consumed)
                if energy_consumed > 0
                else 0
            )
            ...
            print(
                f"  Step {unwrapped_env.current_step:>4}: Reward={cumulative_reward:>10.1f}, "
                f"Battery={battery_pct:>5.1f}%, Coverage={coverage_pct:>5.1f}%, "
...
def plot_trajectories(
    env, dqn_trajectory, greedy_smart_trajectory, greedy_dumb_trajectory
):
    """
...
def plot_comparative_analysis(dqn_df, greedy_smart_df, greedy_dumb_df):
    ...
    ax1.plot(
        greedy_smart_df["step"],
        greedy_smart_df["cumulative_reward"],
        color="#d62728",
        linewidth=2.5,
        linestyle="-",
    ...
    ax1.plot(
        greedy_dumb_df["step"],
        greedy_dumb_df["cumulative_reward"],
        color="gray",
        linewidth=2,
        linestyle=":",
        alpha=0.8,
    ...
    ax2.plot(
        greedy_smart_df["step"],
        greedy_smart_df["battery_percent"],
        color="black",
        linewidth=1.5,
        linestyle="--",
        alpha=0.4,
    ...
    plt.title(
        "Comparative Performance: Deep Reinforcement Learning vs. Heuristics",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
...
def main():
    ...
    if DQN_MODEL_PATH.exists():
        try:
            ...
            # ★ NEW: Save sensor snapshot for DQN
    ...
    # ★ NEW: Save sensor snapshot for Smart Greedy
    save_sensor_snapshot(
        env,
    ...
    # ★ NEW: Save sensor snapshot for Nearest Greedy
    save_sensor_snapshot(
        env,
    ...
    if df_dqn is not None and not df_dqn.empty:
        ...
        print(
            f"  Data Collected: {df_dqn['total_data_collected'].iloc[-1]:>11.0f} bytes"
        )
    ...
    # ★ NEW: Final summary of generated files
    print("\n" + "=" * 100)
    ...
    print("\n📸 JSON Sensor Snapshots (for fairness & heatmap analysis):")
    ...
    print("\n🚀 NEXT STEP: Run analysis scripts to generate dissertation plots:")
