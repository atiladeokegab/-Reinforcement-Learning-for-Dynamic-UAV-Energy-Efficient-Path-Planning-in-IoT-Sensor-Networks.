"""
DQN v7 — GNN + k=10 + Position Features + Tuned Hyperparameters
================================================================
Builds on v6 (GNN + sensor positions) with tuned hyperparameters:
  - buffer_size:            300,000  (was 150k — more diverse experience)
  - batch_size:             128      (was 256  — more stable gradient updates)
  - exploration_fraction:   0.35     (was 0.25 — more exploration for dwell strategy)
  - target_update_interval: 2,500    (was 5000 — faster Q-target adaptation to EMA-ADR)
  - net_arch:               [512, 512, 256] (higher capacity policy head)

Saves to models/dqn_v7_tuned/
...
SAVE_DIR = Path(__file__).parent.parent.parent.parent / "models" / "dqn_v7_tuned"
LOG_DIR  = Path(__file__).parent.parent.parent.parent / "logs"  / "dqn_v7_tuned"
...
class DomainRandEnvV7(DomainRandEnv):
...
def main():
    print("=" * 70)
    print("DQN v7 — GNN + positions + tuned hyperparameters")
    print("=" * 70)
    ...
    checkpoint_cb = CheckpointCallback(
        save_freq=25_000, save_path=str(SAVE_DIR), name_prefix="dqn_v7",
    )
