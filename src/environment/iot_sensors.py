...
class IoTSensor:
    ...
    def collect_data(
        self, uav_position: Tuple[float, float], collection_duration: float = 1.0
    ) -> Tuple[float, bool]:
        # Use simple model for binary range check
        success_prob = self.get_success_probability(
            uav_position, use_advanced_model=False
        )
    ...
    def update_spreading_factor(
        self, uav_position: Tuple[float, float], current_step: int = 0
    ):
        """
        ...
        if self.avg_rssi is None:
            ...
            self.avg_rssi = (self.adr_lambda * current_rssi) + (
                (1 - self.adr_lambda) * self.avg_rssi
            )
        else:
            self.avg_rssi = current_rssi
    ...
    def get_sf_changes(self) -> int:
        ...
        return sum(
            1
            for i in range(1, len(self.sf_history))
...
if __name__ == "__main__":
    ...
    print("-" * 100)
    print(
        f"{'Step':<6} {'Position':<15} {'Current RSSI':<15} {'Avg RSSI':<15} {'SF':<5}"
    )
    print("-" * 100)
    ...
    for step in range(15):
        ...
        print(
            f"{step:<6} ({uav_x:>5.1f}, 25.0) {sensor.current_rssi:>14.1f} dBm {sensor.avg_rssi:>14.1f} dBm {sensor.spreading_factor:<5}"
        )
    ...
    for step in range(10):
        ...
        print(
            f"{step:<6} SF{sensor_inst.spreading_factor:<18} SF{sensor_ema.spreading_factor:<18}"
        )
